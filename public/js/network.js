...
function layout(){
  ...
  const mode = (W < 720 && TWEAKS.layout === 'linear') ? 'stack' : TWEAKS.layout;
  ...
  if(mode === 'radial'){
    ...
    const padY    = Math.max(W < 720 ? 55 : 110, H * (W < 720 ? 0.08 : 0.12));
    const usableH = H - padY*2;
    const padX    = Math.max(W < 720 ? 44 : 140, W * 0.10);
...
function draw(time){
  ...
  if(TWEAKS.layout !== 'radial'){
    const effectiveMode = (W < 720 && TWEAKS.layout === 'linear') ? 'stack' : TWEAKS.layout;
    LAYERS.forEach((L) => {
      const layerX  = L._nodes[0].x0;
      const topNodeY = Math.min(...L._nodes.map(n => n.y0));
      const anchorY = effectiveMode === 'stack' ? L._nodes[0].y0 - 40 : (topNodeY - 64);
...
function bindNetworkEvents(){
  ...
  // ---- Touch events (mobile) — pan, pinch-zoom, tap ----
  let touchStart = null, touchViewStart = null, pinchDist0 = null, pinchViewK0 = null, isTap = false;

  function tapNode(cx, cy){
    let best = null, bestD = Infinity;
    for(const n of nodes){
      const np = nodeScreenPos(n);
      const d = Math.hypot(np.x - cx, np.y - cy);
      const threshold = Math.max(24, 32 * (n.scale || 1)) * view.k;
      if(d < threshold && d < bestD){ bestD = d; best = n; }
    }
    if(!best) return;
    if(best.anchor) openPanel(best);
    else if(best.named){
      const parent = LAYERS[best.layer];
      const anchorNode = nodes.find(x => x.anchor && x.anchor.key === parent.anchor.key);
      if(anchorNode) openPanel(anchorNode, { focus: best.named.key });
    } else if(best.aside){
      openQuotePanel(best);
    }
  }
  ...
  canvas.addEventListener('touchstart', (e) => {
    ...
    if(e.touches.length === 1){
      ...
      touchViewStart = { x: view.x, y: view.y };
      pinchDist0    = null;
      isTap         = true;
  ...
  canvas.addEventListener('touchmove', (e) => {
    ...
    if(e.touches.length === 1 && touchStart){
      ...
      if(Math.hypot(dx, dy) > 8) isTap = false;
      view.x = touchViewStart.x + dx;
      view.y = touchViewStart.y + dy;
      viewTarget.x = view.x; viewTarget.y = view.y; viewTweening = false;
      mouse.x = t.clientX; mouse.y = t.clientY;
      ...
      const k1   = Math.max(0.45, Math.min(2.4, pinchViewK0 * (dist / pinchDist0)));
      const midX = (e.touches[0].clientX + e.touches[1].clientX) / 2;
      const midY = (e.touches[0].clientY + e.touches[1].clientY) / 2;
      const cxOff = midX - W/2 - view.x;
      const cyOff = midY - H/2 - view.y;
      const ratio = k1 / view.k;
      view.k = k1;
      view.x = midX - W/2 - cxOff * ratio;
      view.y = midY - H/2 - cyOff * ratio;
      viewTarget.x = view.x; viewTarget.y = view.y; viewTarget.k = view.k; viewTweening = false;
    }
  }, { passive: false });
  ...
  canvas.addEventListener('touchend', (e) => {
    ...
    if(isTap && e.changedTouches.length === 1){
      ...
      tapNode(t.clientX, t.clientY);
    }
    if(e.touches.length === 0){ touchStart = null; pinchDist0 = null; isTap = false; }
  }, { passive: false });
}
